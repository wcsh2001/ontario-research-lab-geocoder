...
This project scrapes public data on Ontario research facilities from the CFI Research Facilities Navigator, geocodes the resulting addresses using the Google Geocoding API, and generates visualizations to explore the provincial research landscape.
...
- Geocodes facility addresses to latitude/longitude using the Google Geocoding API.
- Exports geocoded facilities as:
  - CSV for further analysis.
  - KML for use in GIS or Google Earth.
  - Interactive HTML map built with Folium.
- Provides a notebook to explore spatial patterns, top institutions, and sectoral distribution across Ontario.

## Project Structure

- `lab_scraper.py`  
  - Discovers all Ontario facility URLs via the Navigator search pages (`province=2100`, 44 pages total as of Jan 2026).
...
- `lab_geocoder.py`  
  ...
  - Adds `Latitude` and `Longitude` columns to the dataset and writes:
    ...
    - `ON_labs_geocoded.kml` – point layer for mapping tools like Google Earth.
...
- This:
  - Crawls all Ontario facility result pages (44 pages as of Jan 2026).
...
- This:
  ...
  - Writes:
    ...
    - `ON_labs_geocoded.kml` for displaying on mapping tools like Google Earth.
...
- Polite delays (`time.sleep`) are added between requests to reduce load on the Navigator servers.
